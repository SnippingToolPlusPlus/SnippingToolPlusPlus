...
public abstract class ClipboardUtilities implements ClipboardOwner
{
	...
	/**
	 * Uploads any image copies/saved to the clipboard to imgur and then sets
	 * the clipboard to the returned url
	 */
	public static void uploadImage()
	{
		...
		if (img == null)
		{
			...
			Main.displayErrorMessage("Upload Error",
					"An image could not be found in the clipboard.");
		} else
			upload = new Upload(img, false);
	}

	/**
	 * Gets the any image currently in the clipboard and returns it as a
	 * BufferedImage
	 * 
	 * @return
	 */
	private static BufferedImage getImageFromClipboard()
	{
				...
				&& transferable.isDataFlavorSupported(DataFlavor.imageFlavor))
		{
			try
			{
				return (BufferedImage) transferable
