...
/**
 * JIntellitype A Java Implementation for using the Windows API Intellitype commands and the RegisterHotKey and UnRegisterHotkey API calls for globally
 * responding to key events. Intellitype are commands that are using for Play, Stop, Next on Media keyboards or some laptops that have those special keys.
 * <p>
...
public final class JIntellitype implements JIntellitypeConstants
{
     ...
     * <p>
     * But the possibility of creation of more instance is only before the instance is created. Since all code defined inside getInstance method is in the
     * synchronized block, even the subsequent requests will also come and wait in the synchronized block. This is a performance issue. The same can be solved
     * using double-checked lock. Following is the implementation of Singleton with lazy initialization and double-checked lock.
     * <p>
     * 
     ...
     * 
     * @param listener
     *            the HotKeyListener to be added
     */
     ...
     * 
     * @param listener
     *            the IntellitypeListener to be added
     */
    ...
    /**
     * Registers a Hotkey with windows. This combination will be responded to by all registered HotKeyListeners. Uses the JIntellitypeConstants for MOD, ALT,
     * CTRL, and WINDOWS keys.
     * <p>
     * 
     * @param identifier
     *            a unique identifier for this key combination
     * @param modifier
     *            MOD_SHIFT, MOD_ALT, MOD_CONTROL, MOD_WIN from JIntellitypeConstants, or 0 if no modifier needed
     * @param keycode
    ...
    /**
     * Registers a Hotkey with windows. This combination will be responded to by all registered HotKeyListeners. Use the Swing InputEvent constants from
     * java.awt.InputEvent.
     * <p>
     * 
     * @param identifier
     *            a unique identifier for this key combination
     * @param modifier
     *            InputEvent.SHIFT_MASK, InputEvent.ALT_MASK, InputEvent.CTRL_MASK, or 0 if no modifier needed
     * @param keycode
     *            the key to respond to in Ascii integer, 65 for A
     */
    ...
    /**
     * Registers a Hotkey with windows. This combination will be responded to by all registered HotKeyListeners. Use the identifiers CTRL, SHIFT, ALT and/or
     * WIN.
     * <p>
     * 
     * @param identifier
     *            a unique identifier for this key combination
     * @param modifierAndKeyCode
     *            String with modifiers separated by + and keycode (e.g. CTRL+SHIFT+A)
     ...
     * 
     * @param identifier
     *            the unique identifer of this Hotkey
     */
    ...
    /**
     * Checks to make sure the OS is a Windows flavor and that the JIntellitype DLL is found in the path and the JDK is 32 bit not 64 bit. The DLL currently
     * only supports 32 bit JDK.
     * <p>
     * 
     ...
     * 
     * @param libraryFile
     *            the java.io.File representing the DLL
     */
     ...
     * 
     * @param command
     *            the unique WM_APPCOMMAND received
     */
    ...
    /**
     * Swing modifier value to Jintellipad conversion. If no conversion needed just return the original value. This lets users pass either the original
     * JIntellitype constants or Swing InputEvent constants.
     * <p>
     * 
     * @param swingKeystrokeModifier
     *            the Swing KeystrokeModifier to check
     ...
     * 
     * @return Map containing key->keycode mapping DOCU Now enables the user to use all keys specified here instead of just [A-Z],[0-9] as before
     */
    ...
    /**
     * Checks if there's an instance with hidden window title = appName running Can be used to detect that another instance of your app is already running (so
     * exit..)
     * <p>
     * 
     * @param appName
     *            = the title of the hidden window to search for
     */
