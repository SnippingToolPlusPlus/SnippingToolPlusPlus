...
import utilities.ClipboardUtilities;
		...
		MouseListener
{
	...
	public EditorPanel(BufferedImage img, Editor e)
	{
		...
		editingLayer = new BufferedImage(img.getWidth(), img.getHeight(),
				BufferedImage.TYPE_INT_ARGB);
		layerG2D = editingLayer.createGraphics();
		layerG2D.setBackground(new Color(0, 0, 0, 0)); // set layer transparent
		layerG2D.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_IN,
				0.0f));
		layerG2D = editingLayer.createGraphics();
		...
		textLayerG2D.setBackground(new Color(0, 0, 0, 0)); // set layer
															// transparent
		textLayerG2D.setComposite(AlphaComposite.getInstance(
	...
	public void setTransparency(int level)
	{
		fillColor = new Color(fillColor.getRed(),
				fillColor.getGreen(), fillColor.getBlue(), level);
	}
	public void setBorderTransparency(int level)
	{
		borderColor = new Color(borderColor.getRed(),
				borderColor.getGreen(), borderColor.getBlue(), level);
	}
...
//		}
	...
	private void drawBorderedRectangle(Graphics2D g)
	{
		...
		g.fillRect((int) (draggedRect().getX() + draggedRect().getWidth()) + (int)(stroke_correction/2),
				(int) draggedRect().getY()  + stroke_correction, 
				stroke_correction, 
				(int) draggedRect().getHeight() - (int)(stroke_correction/2));
		g.fillRect((int) (draggedRect().getX() + stroke_correction), 
				(int) (draggedRect().getY() + draggedRect().getHeight()) + (int)(stroke_correction/2),
				(int) draggedRect().getWidth() + (int)(stroke_correction/2),
				stroke_correction);
		/*
		 * End Shadow
		 */
		...
		g.setColor(fillColor);
		g.fill(new Rectangle(draggedRect().x + (int)(stroke_correction/2), draggedRect().y + (int)(stroke_correction/2), draggedRect().width - (int)(stroke_correction/2), draggedRect().height - (int)(stroke_correction/2)));
	...
	public void backspaceDrawText()
	{
		...
		if (drawText.length() > 0)
		{
			...
			{
				currentChar = drawText.substring(drawText.length() - 1,
						drawText.length());
				nextChar = drawText.substring(drawText.length() - 2,
						drawText.length() - 1);
				...
				System.out
						.println(drawText.substring(0, drawText.length() - 1));
