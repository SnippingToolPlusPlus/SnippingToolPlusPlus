...
public final class JIntellitype implements JIntellitypeConstants
{
	 ...
	 */
	private void fromJarToFs(String jarPath, String filePath)
			throws IOException
	{
	 ...
	 * in the synchronized block, even the subsequent requests will also come
	 * and wait in the synchronized block. This is a performance issue. The same
	 * can be solved using double-checked lock. Following is the implementation
	 * of Singleton with lazy initialization and double-checked lock.
	 * <p>
	 * 
	 ...
	 *            String with modifiers separated by + and keycode (e.g.
	 *            CTRL+SHIFT+A)
	...
	public void registerHotKey(int identifier, String modifierAndKeyCode)
	{
		...
		for (int i = 0; i < split.length; i++)
		{
			...
			} else if ("CTRL".equalsIgnoreCase(split[i])
					|| "CONTROL".equalsIgnoreCase(split[i]))
			{
	...
	private HashMap<String, Integer> getKey2KeycodeMapping()
	{
		...
		map.put("inverted_exclamation_mark",
	...
	private synchronized native void regHotKey(int identifier, int modifier,
			int keycode) throws UnsatisfiedLinkError;

	private synchronized native void terminate() throws UnsatisfiedLinkError;

	private synchronized native void unregHotKey(int identifier)
			throws UnsatisfiedLinkError;

	/**
	 * Checks if there's an instance with hidden window title = appName running
	 * Can be used to detect that another instance of your app is already
	 * running (so exit..)
	 * <p>
	 * 
	 * @param appName
