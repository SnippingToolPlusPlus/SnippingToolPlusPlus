...
public abstract class Notification extends JFrame
{
	...
	protected Shape frameShape = new RoundRectangle2D.Double(0, 0,
	 ...
	 * <p>
	 * Creates the notification box. Must always be called with each new
	 * Notification instance. This method must be called any time the default
	 * variables are changed or else changes will not be made.
	 */
	...
	protected void setPosition()
	{
		...
		Insets screenMax = getToolkit().getScreenInsets(
							...
							// default locations
		{
			...
			if (os.contains("Windows"))
			{
				...
				int taskbarYlocation = getToolkit().getScreenSize().height
						- taskbarSize;
				balloonPosition = new Point(getToolkit().getScreenSize().width
						- getWidth() - margin, taskbarYlocation - getHeight()
						- margin);
			...
			} else if (os.contains("Mac"))
			{
				...
				balloonPosition = new Point(getToolkit().getScreenSize().width
						- getWidth() - margin, taskbarYlocation + margin);
				setLocation(offScreenPosition, balloonPosition.y);
			}
		} else
		// display at whereve the component is located on the screen
		{
			...
			balloonPosition = new Point(comp.getX(), comp.getY() - getHeight()
					- margin);
			setLocation(offScreenPosition, balloonPosition.y);
		}
