...
public class PastebinUploader extends Thread
{
	...
	public void run()
	{
		Main.displayInfoMessage("Uploading Clipboard Text...",
				"Your upload will be done shortly.");
		...
		if (uploaded)
		{
			...
			Main.displayErrorMessage("Error Uploading!", "Error Code: "
					+ pastebinError);
	...
	private boolean uploadToPastebin(String uploadText2)
	{
		try
		{
			...
			data += "&" + URLEncoder.encode("api_paste_expire_date", "UTF-8")
					+ "=" + URLEncoder.encode("1M", "UTF-8");
			data += "&" + URLEncoder.encode("api_paste_code", "UTF-8") + "="
					+ URLEncoder.encode(uploadText2, "UTF-8");
