...
public class Upload extends Thread
{
	...
	public void run()
	{
		...
		if (os.indexOf("Mac") >= 0)
		{
			Main.trayIcon.setImage(new ImageIcon(this.getClass().getResource(
					"/images/uploadMac.png")).getImage());
		} else
		{ // Windows
			Main.trayIcon.setImage(new ImageIcon(this.getClass().getResource(
					"/images/upload.gif")).getImage());
		}
		Main.displayInfoMessage("Uploading...",
		...
		if (uploaded)
		{
			if (Preferences.AUTO_SAVE_UPLOADS)
			{
				...
				new LinkDataSaver(imgUrl[0], delUrl[0], "upload("
						+ Preferences.TOTAL_SAVED_UPLOADS + ")");
			}

			if (!reddit)
			{
				ClipboardUtilities.setClipboard(imgUrl[0]);
				Main.displayInfoMessage("Upload Successful!",
						"Link has been copied to your clipboard");
			} else
			{
				OpenBrowser.openToReddit(imgUrl[0]);
				Main.displayInfoMessage("Upload Successful!",
						"Submitting link to Reddit");
			}

			SoundNotifications.playDing();
			
		} else
			Main.displayErrorMessage("Upload Failed!",
		...
		{
			Main.trayIcon.setImage(new ImageIcon(this.getClass().getResource(
					"/images/trayIconMac.png")).getImage());
		} else
		{ // Windows
			Main.trayIcon.setImage(new ImageIcon(this.getClass().getResource(
					"/images/trayIcon.png")).getImage());
		}
	...
	public String uploadImageFile(File file, String type)
	{
		...
		return "Error uploading file type: ." + type
